...
# json? : 데이터 저장 및 주고 받을때 사용하는 가장 대표적인 텍슽트 기반 데이터 형식. 언어독립성(모든 프로그램 언어에서 기본으로 지원). 키-밸류 구조. 
# 왜 씀? : 프로그램 실행 중 메모리에 있는 객체(퀴즈 클래스 인스턴스)는 그대로 파일로 저장못함. 객체를 문자열로 바꾸어 저장. 나중에 다시 객체로 복원하여 사용.
